...
# 绘制每个数据集的图表
datasets = ['AmazonBook', 'LastFM', 'ML20M']
...
for i, dataset in enumerate(datasets):
    ...
    # 使用红色绘制 ndcg 数据
    ndcg_line, = ax2.plot(data['weight'], data[f'ndcg_{dataset}'] * 100, label='NDCG@20', color='red', marker='x',
                          linewidth=2)

    # 设置坐标轴标签
    ax1.set_xlabel('λ', fontsize=16)  # 设置 x 轴标签字体大小为 16
    ax1.set_ylabel('Recall (%)', color='blue', fontsize=16)  # 设置 y 轴标签字体大小为 16
    ax2.set_ylabel('NDCG (%)', color='red', fontsize=16)  # 设置 y 轴标签字体大小为 16

    # 设置标题
    ax1.set_title(f'{dataset}', fontsize=18)  # 设置标题字体大小为 18

    # 添加图例
    ax1.legend(handles=[recall_line], loc='upper left', fontsize=12)  # 设置图例字体大小为 12
    ax2.legend(handles=[ndcg_line], loc='upper right', fontsize=12)  # 设置图例字体大小为 12
...
# 调整布局
plt.tight_layout()
plt.savefig('model_performance.png', dpi=200)  # 提高保存的分辨率
plt.show()