...
from map2binary import class_map, class_map_5_parts, class_map_parts_mr, class_map_parts_headneck_muscles, map_own_taskid_to_partname
...
# 根据传入的任务名称，获取相应的配置参数（添加新的训练模型后，需要进行相关配置）
def get_config(task):
    crop_addon = [3, 3, 3]
    if task == "total":
    ...
    # 根据训练的模型继续添加配置。比如
    # elif task == "own_colon":
    #     task_id = [999]     # 转换后的onnx模型名称
    #     resample = [0.75, 0.75, 1.0]     # 预处理文件夹中，训练计划中的重采样参数
    #     crop = None     # 裁剪掩码默认为None，表示不进行裁剪
    #     folds = [0]      # 采用的训练计划中的折叠参数
    #     trainer = "nnUNetTrainer_DASegOrd0_NoMirroring"      # 计划中的训练文件夹名称
    #     model = "3d_fullres_high"                             # 计划中的模型名称
    elif task == "small_intestine":     # 以small_intestine为例，添加配置
        task_id = [800]     # 自定义训练的模型，从800开始
        resample = [0.75, 0.75, 1.0]     # 模型计划的空间间距，参考训练后处理.md的    说明
        crop = None     # 裁剪掩码默认为None，表示不进行裁剪
        folds = [0]      # 采用的训练计划中的折叠参数，默认使用0折
        model = "3d_fullres_high"                             # 计划中的模型名称

    # 在使用导出的onnx后不需要定义trainer
...
def get_resource_path(relative_path):
    # 如果在pyinstaller打包的exe文件中运行，则需要使用以下代码获取资源路径，否则返回程序所在目录
    try:
...
def main():
    ...
    base_dir = get_resource_path("results")         # 资源文件路径
