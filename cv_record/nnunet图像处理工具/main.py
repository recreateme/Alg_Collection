    ...
    def run(self):
        try:
            ...
            for patient_idx, patient_dir in enumerate(patient_dirs):
                ...
                # case 1: 包含一个文件夹和许多dicom文件
                if len(subfolders) == 1 and len(files) > 0:
                    folder_A = subfolders[0]
                    folder_A_path = os.path.join(patient_dir, folder_A)

                    # 查找RT结构文件
                    rtstruct_files = [f for f in os.listdir(folder_A_path) if
                                      f.lower().startswith('rs') and f.endswith('.dcm')]
                    if rtstruct_files:
                        rtstruct_found = True
                        rtstruct_path = os.path.join(folder_A_path, rtstruct_files[0])
                        rtss = pydicom.dcmread(rtstruct_path)
                        for roi in rtss.StructureSetROISequence:
                            all_labels.add(roi.ROIName.lower())

                        # 创建dicom文件夹并移动.dcm文件
                        dicom_folder = os.path.join(patient_dir, "dicom")
                        os.makedirs(dicom_folder, exist_ok=True)

                        for file in files:
                            if file.endswith('.dcm'):
                                shutil.move(os.path.join(patient_dir, file), dicom_folder)
                        ...
                        deleted_patients += 1
                        continue
                # case 2: 两个文件夹且一个为dicom
                elif len(subfolders) == 2 and len(files) == 0:
                    if subfolders[0].lower().startswith('dicom'):
                        dicom_folder = subfolders[0]
                        other_folder = subfolders[1]
                    elif subfolders[1].lower().startswith('dicom'):
                        dicom_folder = subfolders[1]
                        other_folder = subfolders[0]
                    else:
                        # 删除该用户数据目录
                        shutil.rmtree(patient_dir)
                        deleted_patients += 1
                    ...
                    other_folder_path = os.path.join(patient_dir, other_folder)
                    # 查找RT结构文件
                    for file in os.listdir(other_folder_path):
                        if file.lower().startswith('rs') and file.endswith('.dcm'):
                            rtstruct_path = os.path.join(other_folder_path, file)
                            rtss = pydicom.dcmread(rtstruct_path)
                            for roi in rtss.StructureSetROISequence:
                                all_labels.add(roi.ROIName.lower())
                            rtstruct_found = True
                ...
                # case 3: 多个文件夹和dicom文件
                elif len(subfolders) > 2 and len(files) > 0:
                    # 对列表subfolders进行排序，找到最新文件夹
                    subfolders.sort(reverse=True)    # 按名称排序
                    latest_folder = subfolders[0]
                    latest_folder_path = os.path.join(patient_dir, latest_folder)

                    #  删除多余文件夹
                    for folder in subfolders:
                        if folder != latest_folder:
                            folder_path_to_delete = os.path.join(patient_dir, folder)
                            try:
                                shutil.rmtree(folder_path_to_delete)
                                deleted_dates += 1
                                print(f"已删除: {folder_path_to_delete}")
                    ...
                    for file in os.listdir(latest_folder_path):
                        if file.lower().startswith('rs') and file.endswith('.dcm'):
                            rtstruct_path = os.path.join(latest_folder_path, file)
                            rtss = pydicom.dcmread(rtstruct_path)
                            for roi in rtss.StructureSetROISequence:
                                all_labels.add(roi.ROIName.lower())
                            rtstruct_found = True
            ...
            # 排序all_labels
            all_labels = set(sorted(list(all_labels)))
...
class DropArea(QWidget):
    ...
    def merge_labels(self):
        ...
        try:
            ...
            # 获取总文件夹数
            patient_folders = os.listdir(self.folder_path)
            ...
            for patient_dir in patient_folders:
                ...
                rtstruct_path = None
                folers = os.listdir(patient_path)
                if folers[0] == "dicom":
                    plans_dirs = os.path.join(patient_path, folers[1])
                else:
                    plans_dirs = os.path.join(patient_path, folers[0])

                # 找到rtstructure文件
                for plans_file in os.listdir(plans_dirs):
                    if plans_file.lower().startswith('rs') and plans_file.endswith('.dcm'):
                        rtstruct_path = os.path.join(plans_dirs, plans_file)
                        break
